 ...
 *                 project, profile
 *
 *  📁 FILE LOCATION — put this file in the SAME folder as
 *     all your HTML files:
 *
 *     indigo-academy/
 *     ├── welcome.html
 ...
 *     <script src="velora.js"></script>
 *
 * ============================================================
 *
 *  HOW THE NAME SYSTEM WORKS:
 *  ─────────────────────────────────────────────────────────
 *  1. User fills the sign-up form on welcome.html
 *  2. VeloraUser.register() saves name → localStorage
 *  3. Every page loads → reads name → updates UI automatically
 *
 *  On welcome.html your form needs:
 *     <form id="signup-modal form"> (already wired below)
 ...
 *     </form>
 *
 *  On profile.html add this attribute to the name heading:
 *     <h1 data-profile-name>Arjun Mehta</h1>
 *  velora.js will replace its text with the saved name.
 * ============================================================
 */
...
const VeloraUser = {
   ...
   * ── THIS IS WHERE THE NAME IS SAVED ──
   * Called with: VeloraUser.register({ name, branch, year, language })
   * The name is stored inside localStorage under VELORA_STORE_KEY.
   * Every page then reads it via VeloraUser.get().name
   */
...
function initWelcomePage() {
  ...
  form.addEventListener('submit', e => {
    ...
    // ✅ NAME IS SAVED HERE — VeloraUser.register() stores it in localStorage
    VeloraUser.register({ name, branch, year, language });

    // Then all other pages read it back with: VeloraUser.get().name
...
/* ─────────────────────────────────────────────
   6. DASHBOARD PAGE — dynamic data + name display
   ───────────────────────────────────────────── */
function initDashboardPage() {
  const user = VeloraUser.get();

  // ── NAME: Update sidebar "Luminescent Scholar" with real name ──
  // Reads from localStorage → replaces placeholder text
  const sidebarNameEl = document.querySelector('nav h3');
  if (sidebarNameEl) sidebarNameEl.textContent = user.name;

  // ── NAME: Update dashboard greeting ──
  // "Evening, Luminescent Scholar" → "Evening, Arjun"
  const greetingEl = document.querySelector('main header h1');
  if (greetingEl) {
    const hour = new Date().getHours();
    const timeGreet = hour < 12 ? 'Good Morning' : hour < 17 ? 'Good Afternoon' : 'Evening';
    greetingEl.textContent = `${timeGreet}, ${user.name.split(' ')[0]}`;
  }

  // ── NAME: Update header top-right profile label ──
  const headerProfileLabel = document.querySelector('header span.text-sm.font-medium');
  if (headerProfileLabel) headerProfileLabel.textContent = user.name.split(' ')[0];

  // Level display
  document.querySelectorAll('p.text-xs.text-slate-500').forEach(el => {
  ...
  // Rank card
  document.querySelectorAll('p.text-sm.font-headline.font-bold').forEach(el => {
    if (el.textContent === 'Top 5%') el.textContent = user.rank;
    if (el.textContent === '42h This Mo.') el.textContent = `${user.timeThisMonth}h This Mo.`;
  });
  ...
  const continueBtn = [...document.querySelectorAll('button')].find(b => b.textContent.trim() === 'Continue Lesson');
  if (continueBtn) continueBtn.addEventListener('click', () => veloraNavigate('editor'));
}
...
function initPracticePage() {
  ...
  // ── NAME: Sidebar name on Practice page ──
  const sidebarNameEl = document.querySelector('nav h3');
  if (sidebarNameEl) sidebarNameEl.textContent = user.name;
...
function initEditorPage() {
  ...
  // ── NAME: Sidebar name on Editor page ──
  const sidebarNameEl = document.querySelector('nav h3');
  if (sidebarNameEl) sidebarNameEl.textContent = user.name;
...
function initProjectPage() {
  ...
  // ── NAME: Sidebar name on Project page ──
  const sidebarNameEl = document.querySelector('nav h3');
  if (sidebarNameEl) sidebarNameEl.textContent = user.name;
...
/* ─────────────────────────────────────────────
   10. PROFILE PAGE — name shown here too
   ───────────────────────────────────────────── */
function initProfilePage() {
  const user = VeloraUser.get();

  // ── NAME: Main profile heading (e.g. "Arjun Mehta") ──
  // Option A — auto-replace any heading that still says default name
  document.querySelectorAll('h1, h2, h3').forEach(el => {
    if (el.textContent.trim() === 'Luminescent Scholar') el.textContent = user.name;
  });

  // Option B — precise targeting (recommended):
  //   Add  data-profile-name  on your <h1> in profile.html:
  //   <h1 data-profile-name>Arjun Mehta</h1>
  ...
  // ── NAME: Weekly Leaderboard "You" row ──
  //   Replace "You" label with first name for personal touch
  document.querySelectorAll('span, p').forEach(el => {
    if (el.textContent.trim() === 'You') el.textContent = user.name.split(' ')[0];
  });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  console.log(`[Velora] ✅ Initialized on page: ${page}`);
>>>>>>> dccb36c6665f32ce5d96d2fd2fe5f7000dd1f75f
});